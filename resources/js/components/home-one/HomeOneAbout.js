                ...
                {data?.map((data, i) => (
                    ...
                    >
                        <div className="service-card service-card-1 radius-small h-100">
                            <div className="inner">
                                <div className="thumbnail" style={{height:'25vh' }}>
                                    <Link
                                     to={data?.category_info ? `/yazilar-detay/${data?.slug}` : `/event-details/${data?.id}`}>
                                        <img
                                            src={data?.image}
                                            alt="Category Thumb"
                                            style={{height:'100%', width:'100%', objectFit:data?.category_info ? 'fill' : 'contain',position:'relative'}}
                                        />
                                        <span className='badge' style={{position:'absolute',right:7,top:'10px',backgroundColor:'black'}}>
                                            {data?.category_info?.name|| 'Yayın'}
                                            </span>
                                    </Link>
                                </div>
                                    ...
                                    <div className="text-center mb-2">
                                            <div className="">
                                                ...
                                                >
                                                    <Link
                                                        className="edu-btn"
                                                        to={data?.category_info ? `/yazilar-detay/${data?.slug}` : `/event-details/${data?.slug}`}
                                                    >
                                                        Devamını Oku{" "}
